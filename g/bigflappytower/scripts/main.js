...
window.DOMElementHandler = class extends self.DOMHandler {
	...
	_OnSetVisible(c) {
		this._autoAttach && (this._elementMap.get(c.elementId).style.display =
			c.isVisible ? "" : "none")
	}
	_OnUpdatePosition(c) {
		if (this._autoAttach) {
...
"use strict";
{
	...
	window.RuntimeInterface = class g {
		...
		IsiOSWebView() {
			...
			return c && I(this._exportType) || navigator.standalone || /crios\/|fxios\/|edgios\//i.test(d)
		}
		async _Init(d) {
		...
		_GetWorkerURL(d) {
			d = this._workerScriptURLs.hasOwnProperty(d) ?
				this._workerScriptURLs[d] : d.endsWith("/workermain.js") && this._workerScriptURLs.hasOwnProperty("workermain.js") ? this._workerScriptURLs["workermain.js"] : "playable-ad" === this._exportType && this._localFileBlobs.hasOwnProperty(d.toLowerCase()) ? this._localFileBlobs[d.toLowerCase()] : d;
			d instanceof Blob && (d = URL.createObjectURL(d));
			return d
		}
		async CreateWorker(d, k, l) {
			if (d.startsWith("blob:")) return new Worker(d, l);
			if ("cordova" === this._exportType && this._isFileProtocol) return d = await this.CordovaFetchLocalFileAsArrayBuffer(l.isC3MainWorker ?
				d : this._scriptFolder + d), d = new Blob([d], {
		...
		async _InitDOM(d, k) {
			...
			this._domHandlers = C.map(q => new q(this));
			this._FindRuntimeDOMHandler();
			var l = d.engineScripts.map(q => "string" === typeof q ? (new URL(q,
		...
		async _MaybeGetCordovaScriptURL(d) {
			return "cordova" === this._exportType && (d.startsWith("file:") || this._isFileProtocol && this.IsRelativeURL(d)) ? (d.startsWith(this._baseUrl) && (d = d.substr(this._baseUrl.length)), d = await this.CordovaFetchLocalFileAsArrayBuffer(d),
				d = new Blob([d], {
					type: "application/javascript"
		...
		async _ConvertDataUriToBlobs(d, k) {
			if ("object" ===
				typeof k) this._localFileBlobs[d] = new Blob([k.str], {
				type: k.type
		...
		_BinaryStringToBlob(d, k) {
			...
			let n = l >> 2,
				...
				t, x;
			for (x = t = 0; t < n; ++t) q[t] = d.charCodeAt(x++) | d.charCodeAt(x++) <<
				8 | d.charCodeAt(x++) << 16 | d.charCodeAt(x++) << 24;
	...
	c.AddDOMHandlerClass(class extends self.DOMHandler {
		...
		_ScheduleSimulatedResize(f, g, d) {
			...
			this._simulatedResizeTimerId = setTimeout(() =>
				this._OnSimulatedResize(f, g, d), 48)
		}
		...
		_OnWrapperMessage(f) {
			"entered-fullscreen" === f ? (c._SetWrapperIsFullscreenFlag(!0), this._OnFullscreenChange()) : "exited-fullscreen" === f ? (c._SetWrapperIsFullscreenFlag(!1), this._OnFullscreenChange()) : console.warn("Unknown wrapper message: ",
				f)
		}
	})
}
...
self.JobSchedulerDOM = class {
	...
	async Init() {
		...
		this._dispatchWorker =
			await this._runtimeInterface.CreateWorker(c, this._baseUrl, {
				name: "DispatchWorker"
			});
...
self.C3AudioBuffer = class {
	...
	static Create(c, a, b, e, h) {
		...
		return !h || c.IsPlayMusicAsSound() || m ? new self.C3WebAudioBuffer(c, a, b, e, h, m) : new self.C3Html5AudioBuffer(c,
			a, b, e, h)
	}
	CreateInstance(c) {
...
self.C3WebAudioBuffer = class extends self.C3AudioBuffer {
	...
	async _Fetch() {
		...
		if ("cordova" === c.GetExportType() && c.IsRelativeURL(this._url) && c.IsFileProtocol()) this._audioData =
...
self.C3WebAudioInstance = class extends self.C3AudioInstance {
	...
	GetPlaybackTime() {
		let c;
		c = this._isPaused ? this._resumePosition : this._playFromSeekPos + (this.GetCurrentTime() -
	...
	Seek(c) {
		this._isStopped || this.HasEnded() || (this._isPaused ?
			this._resumePosition = c : (this.Pause(), this._resumePosition = c, this.Resume()))
	}
	GetResumePosition() {
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
	...
	_OnNavigate(c) {
		...
		if ("back" === a) navigator.app && navigator.app.backHistory ? navigator.app.backHistory() : window.history.back();
			...
			const b = c.target;
			c = c.exportType;
			self.cordova && self.cordova.InAppBrowser ? self.cordova.InAppBrowser.open(a, "_system") : "preview" === c || "windows-webview2" === c ? window.open(a, "_blank") : this._isScirraArcade || (2 === b ? window.top.location = a : 1 === b ? window.parent.location = a : window.location = a)
		} else "new-window" === a && (a = c.url, c = c.tag, self.cordova && self.cordova.InAppBrowser ? self.cordova.InAppBrowser.open(a, "_system") : window.open(a, c))
	}
	_OnRequestFullscreen(c) {
		if ("windows-webview2" ===
			...
			c.requestFullscreen ? c.requestFullscreen(a) : c.mozRequestFullScreen ? c.mozRequestFullScreen(a) : c.msRequestFullscreen ? c.msRequestFullscreen(a) : c.webkitRequestFullScreen && ("undefined" !== typeof Element.ALLOW_KEYBOARD_INPUT ?
				c.webkitRequestFullScreen(Element.ALLOW_KEYBOARD_INPUT) : c.webkitRequestFullScreen())
		}
	}
	_OnExitFullscreen() {
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
	...
	_OnSetWindowMaxSize(c) {
		this.GetWindow().setMaximumSize(c.width,
			c.height)
	}
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
	constructor(c) {
		...
		this.AddRuntimeMessageHandler("vibrate", a => this._OnGamepadVibrate(a));
		this.AddRuntimeMessageHandler("reset-vibrate", a => this._OnGamepadResetVibrate(a));
		window.addEventListener("gamepadconnected", a => this._OnGamepadConnected(a));
		window.addEventListener("gamepaddisconnected", a => this._OnGamepadDisconnected(a));
		window.addEventListener("unload", () => this._OnWindowUnload())
	}
	_GetActiveGamepads() {
		return Array.from(navigator.getGamepads()).filter(c => c && c.connected)
	}
	_OnTestIsSupported(c) {
		return this._isSupported
	}
	_OnReady() {
		this._isReady = !0;
		for (const c of this._GetActiveGamepads()) this.PostToRuntime("gamepad-connected", {
			index: c.index,
			id: c.id
		});
	...
	_OnGamepadDisconnected(c) {
		this._isReady && this.PostToRuntime("gamepad-disconnected", {
			index: c.gamepad.index
		})
	}
	Tick() {
		var c = this._GetActiveGamepads();
		c.length && (c = c.map(a => ({
			index: a.index,
			id: a.id,
			...
			axes: a.axes
		})), this.PostToRuntime("input-update", c))
	}
	_GetGamepadByIndex(c) {
		for (const a of this._GetActiveGamepads())
			if (a.index === c) return a;
		return null
	}
	async _OnGamepadVibrate(c) {
		var a =
			this._GetGamepadByIndex(c.index);
		if (a && (a = a.vibrationActuator) && a.playEffect) try {
			await a.playEffect("dual-rumble", {
				duration: c.duration,
				startDelay: 0,
				weakMagnitude: c.weakMag,
				strongMagnitude: c.strongMag
			})
		} catch (b) {
			console.warn("[Gamepad] Failed to vibrate gamepad: ", b)
		}
	}
	_OnGamepadResetVibrate(c) {
		(c = this._GetGamepadByIndex(c.index)) && (c = c.vibrationActuator) && c.reset && c.reset()
	}
	_OnWindowUnload() {
		for (const c of this._GetActiveGamepads()) {
			const a = c.vibrationActuator;
			a && a.reset && a.reset()
		}
	}
});
